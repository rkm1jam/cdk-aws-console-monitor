# Source https://github.com/matthew-harper/pyCloudTrailProcesser/tree/master
import json
import urllib.parse
import boto3
import io
...
def post_to_sns(user, event, account) -> None:
    message = f'Manual AWS Change Detected in Account {account}:  {user} --> {event}'
    subject = f'Changes made via AWS Console in Account {account}'
    sns_publish(message, subject)


def post_to_sns_details(message) -> None:
    message = {"Manual AWS Change Detected": message}
    subject = f'Changes made via AWS Console - Details'
    sns_publish(message, subject)


def sns_publish(message, subject) -> None:
        ...
        try:
            ...
            with gzip.GzipFile(fileobj=io.BytesIO(content), mode='rb') as fh:
                ...
                for item in output_dict:
                    post_to_sns(get_user_email(item['userIdentity']['principalId']), item['eventName'], item['recipientAccountId'])
